...
export default async function TendersPage() {
  ...
  const analyses = await listAnalyses(user.userId, 50);

  return (
    <>
      <div className="dashboard-heading">
        <div>
          <span className="section-kicker">Мої перевірки</span>
          <h1>Журнал рішень.</h1>
          <p>Зафіксовані оцінки з датою, режимом і версією даних.</p>
        </div>
        <Link href="/analyze" className="button button--primary">Новий аналіз</Link>
      </div>
          ...
          <div className="dashboard-table dashboard-table--wide">
            {analyses.map((item) => (
              <Link href={`/dashboard/tenders/${item.id}`} key={item.id}>
                ...
                <i>{item.mode === "ai-enhanced" ? "AI + PDF" : "Структурований"}</i>
                <time>{new Intl.DateTimeFormat("uk-UA").format(new Date(item.createdAt))}</time>
              </Link>
            ))}
          </div>
        ) : (
          <div className="empty-state">
            <h3>Перевірок ще немає</h3>
            <Link href="/analyze" className="button button--dark">Почати</Link>
          </div>
        )}
      </section>
    </>
