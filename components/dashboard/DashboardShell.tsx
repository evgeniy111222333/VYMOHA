import { BarChart3, Building2, FileArchive, LayoutDashboard, LogOut, Radar, SearchCheck, Settings } from "lucide-react";
...
export function DashboardShell({ user, children }: { user: ChatGPTUser; children: React.ReactNode }) {
  return (
    <div className="dashboard-shell">
      <aside className="dashboard-sidebar">
        <div className="dashboard-sidebar__top"><Logo /><span className="dashboard-sidebar__badge">beta</span></div>
        <nav aria-label="Навігація кабінету">{links.map((link) => <a key={link.href} href={link.href}><link.icon size={18} />{link.label}</a>)}</nav>
      ...
      <div className="dashboard-main">
        <header className="dashboard-topbar"><div><span>Робочий простір</span><b>Моя компанія</b></div><div className="dashboard-user"><span>{initials(user.displayName)}</span><div><b>{user.displayName}</b><small>{user.email}</small></div></div></header>
